...
export async function think(task, agent, memContext = '', dialog = '') {
  ...
  const systemPrompt =
    ...
    `Отвечай на русском, тепло и по-человечески — как живой дружелюбный собеседник, а не как инструкция или робот. ` +
    `Будь лёгким и по делу: без долгих вступлений, без занудства, морализаторства и «воды», не повторяйся и не растекайся. ` +
    `Уместный лёгкий юмор и эмодзи в меру — только в плюс. ` +
